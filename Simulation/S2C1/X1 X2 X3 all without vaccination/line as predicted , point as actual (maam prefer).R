...
#     zaxis = list(title = 'Disease Count', range=c(0,17000))
#   ))
# 
...
#       zaxis = list(title = 'Disease Count', showgrid = FALSE,range=c(0,17000))
#     ),
#     legend = list(x = 0.6, y = 0.9,  # Move legend inside the plot (top-right)
#                   font = list(size = 6))  # Move legend inside the top-right corner
#   )
# 
# 
# 
...
plot_ly(colors = c("red", "blue", "green")) %>%  # Manually define colors for three regions
  add_trace(
    data = data, 
    x = ~time, 
    y = ~region, 
    ...
    type = 'scatter3d', 
    mode = 'lines', 
    color = ~as.factor(region),  # Still needed for mapping, but colors are overridden
    name = ~paste0("Region ", region, " - Predicted Count"),
    line = list(width = 4)
  ) %>%
  add_trace(
    x = time, 
    ...
    marker = list(
      color = "red",  # Same color as Region 1
      size = 1,  
      symbol = 'star'  
    )
  ) %>%
  add_trace(
    ...
    marker = list(
      color = "blue",  # Same color as Region 2
      size = 2,  
      symbol = 'star'
    )
  ) %>%
  add_trace(
    ...
    marker = list(
      color = "green",  # Same color as Region 3
      size = 2,  
      symbol = 'star'
    )
  ) %>%
  layout(
    scene = list(
      ...
      yaxis = list(title = 'Regions', tickvals = c(1, 2, 3), ticktext = c('Region-1', 'Region-2', 'Region-3'), showgrid = FALSE,
                   titlefont = list(size = 17)),
      zaxis = list(title = 'Disease Count', showgrid = FALSE, range = c(0, 17000),
                   tickvals = seq(2000, 16000, by = 2000),
                   ticktext = paste0(seq(2, 16, by = 2), "k"),
                   titlefont = list(size = 17))
    ),showlegend = FALSE,
...
#       xaxis = list(title = 'Time (in biweeks)', showgrid = FALSE),
#       yaxis = list(title = 'Regions', tickvals = c(1, 2, 3), ticktext = c('Region-1', 'Region-2', 'Region-3'), showgrid = FALSE),
#       zaxis = list(title = 'Disease Count', showgrid = FALSE,range=c(0,14000))
#     ),
#     legend = list(x = 0.6, y = 0.9,  # Move legend inside the plot (top-right)
#                   font = list(size = 6)),
