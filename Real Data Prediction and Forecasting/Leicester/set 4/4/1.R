...
colnames(data) <- c("S","F", "Lag1_and_Lag1_2nd_and_LAg1_3rd","biweeklybirth","Seasonalit1","Seasonalit2","time","baby_boom")
head(data,20)

model <- glm(cbind(S,F) ~ Lag1_and_Lag1_2nd_and_LAg1_3rd+biweeklybirth+Seasonalit1+Seasonalit2+time+baby_boom, 
...
model <- glm(cbind(S,F) ~ Lag1_and_Lag1_2nd_and_LAg1_3rd+biweeklybirth+Seasonalit1+Seasonalit2+time+baby_boom+e_lag1+e_lag2, 
             family = binomial(link = "logit"), data = data)
