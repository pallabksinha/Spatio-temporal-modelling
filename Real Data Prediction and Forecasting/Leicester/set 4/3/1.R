...
training_upto<-546
infected_first_category <- data$Leicester[1:training_upto]
...
#distance between [leicester,coventry]=52.95513,[Leicester,birmingham]=52.16724 (in kms)
#so exp(-52.95513) = 1.004337e-23 ; exp(-52.16724) = 2.208287e-23
# (1/52.95513)^2=0.0003566021 ; (1/52.16724)^2 = 0.0003674551
...
xn<-as.numeric(birth_data[25, 3:((training_upto/26)+2)])
...
# colnames(data) <- c("Intercept", "Lag1+Lag1_2nd+LAg1_3rd","biweeklybirth","Seasonalit1","Seasonalit2","time","baby_boom")
# head(data,20)
# 
# 
...
colnames(data) <- c("S","F","Lag1_and_Lag1_2nd_and_LAg1_3rd",
                    "biweekly_birth_data_in_inverselogit","seasonality1","seasonality2","t","baby_boom_effect")
head(data,20)

model <- glm(cbind(S,F) ~ Lag1_and_Lag1_2nd_and_LAg1_3rd+
               biweekly_birth_data_in_inverselogit+seasonality1+seasonality2+t+baby_boom_effect, 
...
error<-infected_first_category_success-predicted

e_lag1 <- dplyr::lag(error,1)
e_lag1[is.na(e_lag1)]<-0

e_lag2<- dplyr::lag(error,2)
e_lag2[is.na(e_lag2)]<-0

length(as.vector(e_lag1))
length(as.vector(e_lag2))

data<-cbind(data,e_lag1,e_lag2)
head(data,20)

model <- glm(cbind(S,F) ~ Lag1_and_Lag1_2nd_and_LAg1_3rd+
               biweekly_birth_data_in_inverselogit+seasonality1+seasonality2+t+baby_boom_effect+e_lag1+e_lag2, 
             family = binomial(link = "logit"), data = data)
...
summary_df <- summary_df[, -((ncol(summary_df) - 1):ncol(summary_df))]
summary_df <- round(summary_df, 5)

setwd("C:/Users/ASUS/Desktop/Bath/paper purpose/Coventry spatial connection/prediction/set 6(1)/Leicester")
library(writexl)
write_xlsx(summary_df, "Best model er model_summary.xlsx")
