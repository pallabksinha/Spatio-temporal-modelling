data<-data[,-c(1,2)]
data<-cbind(rep(1,length(data$seasonality2)),data)
...
par(mfrow = c(2, 2))
acf(model$residuals   ,xlab="Lag",main="Acf plot for errors",ci=0.995)
pacf(model$residuals,   xlab="Lag",main="PAcf plot for errors",ci=0.995,ylim=c(-1,1))
plot(residuals_rq,ylim = c(-3,3),ylab = "Residuals")
